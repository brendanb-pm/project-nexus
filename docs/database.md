...
NX-3.4 adds an optional `handoffs.submission_key` with a per-assignment unique index. A submitted handoff retains its server-derived assignment context, trusted submission timestamp, and append-only audit event. The optional column preserves compatibility with pre-existing draft or historical handoffs; retry deduplication applies to new submitted handoffs.
