...
NX-3.3 adds optional `incident_reports.reported_by_user_id` and `submission_key`. New reports record the authoritative authenticated user and use the assignment/key unique index for safe retry; legacy incident rows remain valid. The migration is additive and can be disabled before production use by removing the new index, foreign key, and columns in reverse order. Incident report submissions and their audit event are written in one transaction.

`daily_activity_reports` and `handoffs` are retained as legacy evidence. They are not new-write authorities: `activity_entries` is the canonical DAR model and `end_of_shift_reports` is the sole new end-of-shift submission model. Historical Handoffs remain readable, reviewable, amendable through the existing immutable revision/audit path, and are never destructively rewritten. See [Reporting domain](reporting-domain.md).
